...
| Sections | 16–32 bars. Micro-changes every 4–8 (a hat in, a filter opening, a send throw) | [start] |
| DJ ends | 16–32 bars of stripped drums at the start and the end | [start] |
| Breakdowns | Can keep the rumble/sub. The "drop" can simply be the kick returning | [start] |
| Edit | Subtract first. Automate sends, cutoff and macros for motion | [start] |
| Budget | 5–7 sounds total is common. Interest comes from varying them, not from adding more | [start] |
...
- **Stutter build:** Beat Repeat last on a playing track (e.g. HATS). Automate **Grid** 1/8→1/32 (slice size) and Chance ~30%→100% over 1–2 bars; Chance 0% elsewhere = no repeats. Built from the track's own material. [manual] parameters · [start] recipe
...
- Beat Repeat only repeats incoming audio: nothing playing through it = silence. [manual] Keep the smallest Grid to the last few beats, or it turns into noise. [start]
