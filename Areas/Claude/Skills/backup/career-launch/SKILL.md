...
> "Where we are: [current roadmap step]. Pending: [the 1–2 things blocking progress]. Want to work on that, or something else?"

Wait for the user's go-ahead before diving into a big chunk of work.

## Step 2 — Hold the context (who, goal, tone)

- **Who:** Ale Gómez — cinema background (directing at ECIB), photographer, now an AI-assisted app & brand builder. Day job: part-time front desk at a tennis club (~€1,000/mo). The unusual mix *cinema → apps → brand* is the **angle**, not a weakness.
- **The goal:** an **assistant / junior / auxiliary** entry role abroad in **fashion & arts**, **design / creative studios & agencies**, **tech / product** (his strongest hard skills — iOS/Swift apps shipped, front/back-end, AI/agentic engineering; favour design-led product + fashion/culture-tech crossover), or photography/visual — *not* the film industry. Music is a *future* interest, not a current selling point (~1–2 months DJ/producing). He feels **stuck in Spain**; the blocker is **visibility + a foot in the door**, not raw ability.
- **Tone (critical):** this work has scared him before. Keep every step **small, concrete, and confidence-building**. Beginner-friendly language, no jargon dumps, no overselling his level. When something feels big, break it into the next single doable action.

## Step 3 — The three workstreams

### A. Portfolio website
- **Code:** `/Users/agomezu/Claude/portfolio/` — plain HTML/CSS/JS, GitHub Pages (repo `github.com/blhdes/portfolio`), domain via Porkbun.
- **Live:** **https://alegomez.studio**.
- **Design:** minimal editorial — warm paper + ink + vermilion accent, Switzer typeface. The site itself is a portfolio piece; keep it feeling *designed*.
- **Sections:** Apps · Photography (10–20 curated shots) · Projects (cinema work + fashion brand) · Bio/About.
- **Copy rule:** short and informational, never long paragraphs. Pattern = `[Medium/what]. [My role]. [Notable context/link].` (see memory `copy-short-informational`).

### B. New work to show
- **Apps (live):** Culla, CullaMusic (both at culla.app). **Pipeline:** Doppio, Warket, Village — add each to the site as it ships.
- **Fashion brand:** launching ~**Aug–Sep 2026**, a planned centerpiece (identity, product, photos).
- **Photography & cinema:** curate, don't dump. E.g. *Mia & Marc* — short film, DOP, distributed on MUBI.

### C. Job search & relocation
Lives in `Projects/Portfolio/Job-Search/` (hub: `Job-Search.md`). A 5-phase pipeline — **work it slowly, never all at once:**
- **Phase 0 Setup** (done) · **Phase 1 Fetch** ← current · **Phase 2 Review/filter** (user) · **Phase 3 Deepen shortlist** · **Phase 4 Contact** 🔒.
- **Phase 1 rule:** mine directories from `sources.md` and fill the `companies-<city>.md` tables, **one city or one industry per session** — no overwhelm. Everything enters as `todo`; don't judge fit for the user (that's Phase 2).
- **City notes** organise by **city → industry → size** (Big/Small/Micro), each row: Company · Industry · Size · Why it fits · Links · Status. Status pipeline: `todo → ★ shortlist → ✗ pass → ✎ drafting → → contacted → ↩ reply`.
- **Seniority:** seeking an **assistant / junior / auxiliary** entry role — not senior.
- **Profile-fit filter:** **fashion & arts**, **design / creative studios & agencies**, **tech / product** (strongest hard experience — iOS/Swift, front/back-end, AI/agentic engineering; favour design-led product cos + fashion/culture-tech crossover), and photography/visual. Honest assets: photography, a cinema-trained visual eye, organised execution, real app/product building + AI skills, languages (ES/CA/EN). **Music is a future interest, not a current hook** (~1–2 months DJ/producing) — keep music companies secondary/aspirational.
- **Cities (priority):** Paris > Amsterdam > Berlin > London > Barcelona/Spain (last resort). Relocation conditions: salary > current ~€1,000/mo, housing affordable **alone** with room for a home studio, English-speaking workplace.
- **Phase-4 gate:** no outreach until the portfolio is a **shareable v1** (needs real screenshots + photos + portrait). Then short personal email + tailored letter per shortlisted company. Never send without confirming.
- Keep the **timeline honest**: site polished ~July 2026 → brand section at launch ~Sep 2026 → applications well before the Dec 2026–Feb 2027 window.

### D. Oposiciones — parallel stability track (started 2026-08-17, Spain-only, separate goal)
...
- **Also published to the live portfolio site, by explicit request:** each study session gets mirrored to `/Users/agomezu/Claude/portfolio/oposiciones/` (e.g. `oposiciones/bloque-ii/index.html`) and deployed with the normal `git push` to the **public** `github.com/blhdes/portfolio` repo. This was a deliberate, confirmed exception — flagged to the user first (public repo, professional domain) before proceeding. Ground rules to keep applying to every future addition here:
  ...
  - Reuses the site's existing design tokens/typeface via a dedicated `.study-*` CSS block in `css/style.css` (added 2026-08-17) — don't invent a new visual language for it.
  - **Bump the `?v=N` cache-bust query param** on `style.css`/`main.js` across *every* HTML page (not just the new one) whenever either file changes — was at `v=75` as of 2026-08-17.
  - Still confirm before every `git push` to this repo, same as any other portfolio deploy.

## Step 4 — Vault save protocol (whenever work advances)

The vault is the safety net for resumability across `/clear`s. After meaningful progress:

1. **Update `Portfolio.md`** — tick/adjust the **Roadmap** checklist and any dates so the state stays honest.
2. **Content note** — for a substantial new piece (a written bio version, a relocation research note, an application log), create a kebab-case `.md` in `Projects/Portfolio/` with frontmatter:
   ```yaml
   ---
   title: [Title Case]
   date: [today, YYYY-MM-DD]
   tags: [portfolio, career, [topic]]
   ---
   ```
...
- **Don't make it feel big or scary** — always reduce to the next concrete step.
- **Don't write long, listy copy** for the site — distill, don't transcribe.
- **Don't oversell** his level or invent credentials; the honest story is the strength.
- **Don't railroad him back into the film industry** — he's deliberately moving away from it.
- **Don't skip the vault writes** — keep `Portfolio.md` the honest source of truth.
- **Don't publish or push** (site deploys, repo pushes, external sends) without confirming first.
- **Don't blend oposiciones into the "creative job abroad" pitch** — it's a parallel, Spain-only stability track with its own separate goal, not a stepping stone or a competing priority. Keep its notes in Spanish; that's intentional, not a mistake to fix.
